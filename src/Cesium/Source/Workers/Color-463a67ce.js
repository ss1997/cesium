...
define(['exports', './RuntimeError-1ff80201', './when-8166c7dd', './Transforms-f4065d1e', './ComponentDatatype-7bff63c1'], (function (exports, RuntimeError, when, Transforms, ComponentDatatype) { 'use strict';
  ...
  Color.fromRandom = function (options, result) {
    ...
    let red = options.red;
    if (!when.defined(red)) {
      const minimumRed = when.defaultValue(options.minimumRed, 0);
      const maximumRed = when.defaultValue(options.maximumRed, 1.0);
    ...
    if (!when.defined(green)) {
      const minimumGreen = when.defaultValue(options.minimumGreen, 0);
      const maximumGreen = when.defaultValue(options.maximumGreen, 1.0);
    ...
    if (!when.defined(blue)) {
      const minimumBlue = when.defaultValue(options.minimumBlue, 0);
      const maximumBlue = when.defaultValue(options.maximumBlue, 1.0);
    ...
    if (!when.defined(alpha)) {
      const minimumAlpha = when.defaultValue(options.minimumAlpha, 0);
      const maximumAlpha = when.defaultValue(options.maximumAlpha, 1.0);
