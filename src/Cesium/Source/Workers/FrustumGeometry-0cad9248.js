...
define(['exports', './Transforms-f4065d1e', './Matrix2-53a99ae0', './RuntimeError-1ff80201', './ComponentDatatype-7bff63c1', './when-8166c7dd', './GeometryAttribute-de44b214', './GeometryAttributes-50becc99', './Plane-d60c1860', './VertexFormat-47fb135f'], (function (exports, Transforms, Matrix2, RuntimeError, ComponentDatatype, when, GeometryAttribute, GeometryAttributes, Plane, VertexFormat) { 'use strict';
  ...
  FrustumGeometry._computeNearFarPlanes = function (
    ...
    let x = when.defaultValue(xDirection, scratchXDirection);
    let y = when.defaultValue(yDirection, scratchYDirection);
    let z = when.defaultValue(zDirection, scratchZDirection);
  ...
  FrustumGeometry.createGeometry = function (frustumGeometry) {
    ...
    if (
      ...
      const normals = when.defined(vertexFormat.normal)
        ? new Float32Array(3 * 4 * numberOfPlanes)
        : undefined;
      const tangents = when.defined(vertexFormat.tangent)
        ? new Float32Array(3 * 4 * numberOfPlanes)
        : undefined;
      const bitangents = when.defined(vertexFormat.bitangent)
        ? new Float32Array(3 * 4 * numberOfPlanes)
        : undefined;
      const st = when.defined(vertexFormat.st)
